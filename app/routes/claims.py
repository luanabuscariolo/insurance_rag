...
@router.post("",response_model=ClaimResponse, status_code=status.HTTP_201_CREATED, summary="Criar novo sinistro",)

async def create_claim(payload:ClaimCreate, service:ClaimService = Depends(get_service),) -> ClaimResponse:
    return await service.create(payload)

@router.get("",response_model=ClaimListResponse, summary="Listar sinistros com filtros opcionais",)
...
@router.get("/{claim_id}", response_model=ClaimResponse, summary="Buscar sinistro por ID")

async def get_claim(claim_id: int, service: ClaimService = Depends(get_service)) -> ClaimResponse:
...
@router.patch("/{claim_id}/status", response_model=ClaimResponse, summary="Atualizar status e valor aprovado")

async def update_claim_status(claim_id: int, payload: ClaimUpdate, service: ClaimService = Depends(get_service)) -> ClaimResponse:
...
@router.delete("/{claim_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Remover sinistro")

async def delete_claim(claim_id: int, service: ClaimService = Depends(get_service)) -> None:
    deleted = await service.delete(claim_id)
    if not deleted:
        raise HTTPException(status_code=404, detail=f"Sinistro {claim_id} não encontrado")


@router.get("/stats/summary", summary="Estatísticas agrupadas por status")
